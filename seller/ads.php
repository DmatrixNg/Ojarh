                                        ...
                                        <tbody>
                                            <?php foreach ($userClass->get_user_ads($_SESSION['userid']) as $key => $value): ?>
                                                <tr role="row" class="odd">
                                                    ...
                                                    <td tabindex="0" class="sorting_1"><a href="<?= BASE_URL.'#ads-'.$value['id'] ?>"> <img src='../public/ads/<?= $value['img'] ?>' width="150" /></a></td>
                                                    <td><?= $value['ads_location'] ?></td>
                                                    <td><?= $value['link'] ?></td>
                                                    <td><?= $value['created'] ?></td>
                                                    <td><?= $value['end_date'] ?></td>
                                                    <td><a href="../api/controllers/stop_ad.php?id=<?= $value['id'] ?>" class="btn btn-danger btn-sm">Stop Ads</a>
                                                      <button class="btn btn-dark btn-sm" onclick="ads_renewal(<?= $value['id'] ?>)">Renew Ads</button></td>
                                                </tr>
                                            <?php endforeach ?>
                                        </tbody>
                                    </table>
