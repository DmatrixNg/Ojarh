...
if($user->product_title == '' || $user->product_category=='' || $user->product_description=='' || $user->productavailability=='' || $user->market=='' || $user->countryorigin=='' || $user->expiration=='' || $user->performance==''){
	header("location: ".BASE_URL."admin/add_product.php?result=One of the field is empty");
	return;
}
...
if($user->product_pack0 == '' || $user->product_price0 == '' || $user->product_discount0 == ''){
	header("location: ".BASE_URL."admin/add_product.php?result=You must add at least one product pack details!");
	return;
}else{
...
// print_r($_POST);
// print_r($user->admin_add_product());
...
if($user->file_name0 == ''){
	header("location: ".BASE_URL."admin/add_product.php?result=You must add at least one product image!");
	return;
}else{
...
if(is_uploaded_file($user->file_tmp0) || is_uploaded_file($user->file_tmp1) || is_uploaded_file($user->file_tmp2) || is_uploaded_file($user->file_tmp3) || is_uploaded_file($user->file_tmp4) || is_uploaded_file($user->file_tmp5) || is_uploaded_file($user->file_tmp6) || is_uploaded_file($user->file_tmp7)){
    if($user->file_size0 > 1097152 || $user->file_size1 > 1097152 || $user->file_size2 > 1097152 || $user->file_size3 > 1097152 || $user->file_size4 > 1097152 || $user->file_size5 > 1097152 || $user->file_size6 > 1097152 ){
    	header('Location: '.BASE_URL.'admin/add_product.php?result=File must not exceed 1MB.');
        return true;
    }else{
    	if($user->admin_add_product()){
    		if ( !is_dir(  BASE_URL."seller/productimg/".$user->productid ) ) {
				mkdir(  BASE_URL."seller/productimg/".$user->productid );
				move_uploaded_file($user->file_tmp0, BASE_URL."seller/productimg/".$user->productid."/".$user->file_name0);
				move_uploaded_file($user->file_tmp1, BASE_URL."seller/productimg/".$user->productid."/".$user->file_name1);
				move_uploaded_file($user->file_tmp2, BASE_URL."seller/productimg/".$user->productid."/".$user->file_name2);
				move_uploaded_file($user->file_tmp3, BASE_URL."seller/productimg/".$user->productid."/".$user->file_name3);
				move_uploaded_file($user->file_tmp4, BASE_URL."seller/productimg/".$user->productid."/".$user->file_name4);
				move_uploaded_file($user->file_tmp5, BASE_URL."seller/productimg/".$user->productid."/".$user->file_name5);
				move_uploaded_file($user->file_tmp6, BASE_URL."seller/productimg/".$user->productid."/".$user->file_name6);

				header('Location: '.BASE_URL.'admin/add_product.php?result=Product added successfully!');
				return true;
			...
			header("location: '.BASE_URL.'admin/add_product.php?result=Fail to add product, try again!");
			return false;
		}
    }

}else{
	header('Location: '.BASE_URL.'admin/add_product.php?result=Please upload at least one product image!');
	return false;
}
