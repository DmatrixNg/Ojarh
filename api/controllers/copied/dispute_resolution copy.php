...
if($user->create_dispute()){
	if(is_uploaded_file($user->file_tmp)){
		if ( !is_dir( "../../seller/disputefile/" ) ) {
			...
			if(move_uploaded_file($user->file_tmp, $targetPath)) {
				header('Location: '.BASE_URL.'seller/dispute_center.php?result=Dispute ticket has been created!');
				return true;
			}
		}else{
			$targetPath = "../../seller/disputefile/".$user->file_name;
	...
	header("Location: ".BASE_URL."seller/dispute_center.php?result=Fail to create dispute ticket!");
	return false;
}
